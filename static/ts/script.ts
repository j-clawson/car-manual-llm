/**
 * Main application script for Car Manual Assistant
 * This TypeScript file provides all client-side functionality including:
 * - PDF uploading and processing
 * - Embedding generation
 * - Search functionality with query handling
 */
document.addEventListener('DOMContentLoaded', function() {
    // API endpoint URL - can be changed if hosting elsewhere
    const apiUrl: string = 'http://localhost:8000';
    
    // Type definitions for API responses
    
    /**
     * Response from the PDF processing endpoint
     * Contains information about the processed file and next steps
     */
    interface ProcessingResponse {
        success: boolean;             // Whether processing was successful
        output_file: string;          // Path to the processed JSON output
        metadata: Record<string, any>; // Document metadata from PDF
        message: string;              // Human-readable status message
        next_embedding_command?: string; // Optional command for next step
    }

    /**
     * Response from the embedding generation endpoint
     * Contains information about generated embeddings
     */
    interface EmbeddingResponse {
        success: boolean;          // Whether embedding was successful
        input_file: string;        // Input JSON file path
        output_file: string;       // Output embeddings file path
        num_embeddings: number;    // Number of embeddings created
        embedding_dimension: number; // Dimension of each embedding vector
        message: string;           // Human-readable status message
    }

    /**
     * Individual search result with text, similarity score, and metadata
     */
    interface SearchResult {
        text: string;               // Text content of the matching chunk
        similarity: number;         // Similarity score (0-1) between query and chunk
        metadata: Record<string, any>; // Metadata about this chunk (page number, etc.)
    }

    /**
     * Response from the search endpoint
     * Contains the query results and generated answer
     */
    interface SearchResponse {
        query: string;           // Original search query
        answer: string;          // Generated answer from fine-tuned model
        results: SearchResult[]; // Array of relevant text chunks
        error: string | null;    // Error message if any occurred
    }
    
    // Get references to DOM elements for the upload form
    const uploadForm: HTMLFormElement = document.getElementById('uploadForm') as HTMLFormElement;
    const uploadStatus: HTMLDivElement = document.getElementById('uploadStatus') as HTMLDivElement;
    const fileNameDisplay: HTMLDivElement = document.getElementById('fileName') as HTMLDivElement;
    const fileInput: HTMLInputElement = document.getElementById('pdfFile') as HTMLInputElement;
    
    // Update file name display when file is selected
    fileInput.addEventListener('change', function() {
        if (fileInput.files && fileInput.files.length > 0) {
            const fileName = fileInput.files[0].name;
            fileNameDisplay.textContent = fileName;
            fileNameDisplay.title = fileName; // For tooltip on hover
            fileNameDisplay.classList.add('file-selected');
        } else {
            fileNameDisplay.textContent = 'No file selected';
            fileNameDisplay.title = '';
            fileNameDisplay.classList.remove('file-selected');
        }
    });
    
    /**
     * Handle PDF upload form submission
     * Sends the PDF to the server, processes it, and generates embeddings
     */
    uploadForm.addEventListener('submit', async function(e: Event) {
        // Prevent default form submission behavior
        e.preventDefault();
        
        // Get the selected file from the file input
        let file: File | null = null;
        if (fileInput.files && fileInput.files.length > 0) {
            file = fileInput.files[0];
        }
        
        // Validate that a file was selected
        if (!file) {
            showStatus('error', 'Please select a PDF file');
            return;
        }
        
        // Create FormData object to send the file
        const formData: FormData = new FormData();
        formData.append('file', file);
        
        // Show upload in progress message
        showStatus('progress', 'Uploading and processing...');
        
        try {
            // Step 1: Upload and process the PDF
            const response: Response = await fetch(`${apiUrl}/process-pdf`, {
                method: 'POST',
                body: formData
            });
            
            const result: ProcessingResponse = await response.json();
            
            if (result.success) {
                // Extract filename from path for the embedding API call
                let jsonFile: string = '';
                const parts = result.output_file.split('/');
                if (parts.length > 0) {
                    jsonFile = parts[parts.length - 1];
                }
                showStatus('progress', 'File processed successfully! Generating embeddings...');
                
                // Step 2: Generate embeddings for the processed file
                const embedResponse: Response = await fetch(`${apiUrl}/generate-embeddings/${encodeURIComponent(jsonFile)}`, {
                    method: 'POST'
                });
                
                const embedResult: EmbeddingResponse = await embedResponse.json();
                
                if (embedResult.success) {
                    // Show success message with embedding count
                    showStatus('success', `
                        File processed and indexed successfully!
                        Embeddings created: ${embedResult.num_embeddings}
                        You can now ask questions about this manual.
                    `);
                    
                    // Scroll to query section
                    const querySection = document.querySelector('.query-section');
                    if (querySection) {
                        setTimeout(() => {
                            querySection.scrollIntoView({ behavior: 'smooth' });
                        }, 500);
                    }
                    
                } else {
                    // Show embedding error
                    showStatus('error', 'Error generating embeddings');
                }
            } else {
                // Show processing error
                showStatus('error', result.message);
            }
        } catch (error) {
            // Handle any exceptions that occurred during the process
            let errorMessage = 'Unknown error';
            if (error instanceof Error) {
                errorMessage = error.message;
            } else if (typeof error === 'string') {
                errorMessage = error;
            }
            showStatus('error', errorMessage);
        }
    });
    
    // Helper function to show status messages
    function showStatus(type: 'success' | 'error' | 'progress', message: string): void {
        const color = type === 'success' ? 'green' : 
                     type === 'error' ? 'red' : 
                     'black';
        
        const messageLines = message.trim().split('\n');
        const formattedMessage = messageLines.map(line => `<p style="color: ${color};">${line.trim()}</p>`).join('');
        
        uploadStatus.innerHTML = formattedMessage;
        
        // Clear status after some time for success messages
        if (type === 'success') {
            setTimeout(() => {
                // Fade out effect
                uploadStatus.style.opacity = '0.5';
                setTimeout(() => {
                    uploadStatus.style.opacity = '1';
                }, 300);
            }, 5000);
        }
    }
    
    // Get references to DOM elements for the search functionality
    const searchButton: HTMLButtonElement = document.getElementById('searchButton') as HTMLButtonElement;
    const queryInput: HTMLInputElement = document.getElementById('queryInput') as HTMLInputElement;
    const loadingIndicator: HTMLDivElement = document.getElementById('loadingIndicator') as HTMLDivElement;
    const resultsSection: HTMLDivElement = document.getElementById('resultsSection') as HTMLDivElement;
    const resultsContainer: HTMLDivElement = document.getElementById('resultsContainer') as HTMLDivElement;
    const answerText: HTMLParagraphElement = document.getElementById('answerText') as HTMLParagraphElement;
    
    /**
     * Handle search button click
     * Sends search query to the server and displays results
     */
    searchButton.addEventListener('click', async function() {
        // Get and validate search query
        const query: string = queryInput.value.trim();
        
        if (!query) {
            alert('Please enter a question');
            return;
        }
        
        // Show loading indicator and hide previous results
        loadingIndicator.style.display = 'block';
        resultsSection.style.display = 'none';
        
        try {
            // Send search request to the server
            const response: Response = await fetch(`${apiUrl}/search`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    query: query,
                    top_k: 3,                // Get top 3 most relevant passages
                    response_format: 'json'  // Request JSON response format
                })
            });
            
            const result: SearchResponse = await response.json();
            
            // Hide loading indicator
            loadingIndicator.style.display = 'none';
            
            // Display the AI-generated answer
            answerText.textContent = result.answer;
            
            // Clear any previous search results
            resultsContainer.innerHTML = '';
            
            // Create and append HTML elements for each search result
            result.results.forEach((item: SearchResult, index: number) => {
                const resultItem: HTMLDivElement = document.createElement('div');
                resultItem.className = 'result-item';
                
                // Format metadata
                let metadataHtml = '';
                if (item.metadata && Object.keys(item.metadata).length > 0) {
                    const metaEntries = Object.entries(item.metadata)
                        .map(([key, value]) => `<strong>${key}</strong>: ${value}`)
                        .join(' | ');
                    metadataHtml = `<p class="metadata">${metaEntries}</p>`;
                }
                
                resultItem.innerHTML = `
                    <h4>Passage ${index + 1}</h4>
                    <p>${item.text.replace(/\n/g, '<br>')}</p>
                    ${metadataHtml}
                    <p><strong>Similarity:</strong> ${(item.similarity * 100).toFixed(2)}%</p>
                `;
                resultsContainer.appendChild(resultItem);
            });
            
            // Show the results section with fade-in effect
            resultsSection.style.opacity = '0';
            resultsSection.style.display = 'block';
            setTimeout(() => {
                resultsSection.style.transition = 'opacity 0.3s ease';
                resultsSection.style.opacity = '1';
                
                // Scroll to results
                resultsSection.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
            }, 10);
            
        } catch (error) {
            // Handle any errors that occurred during search
            loadingIndicator.style.display = 'none';
            let errorMessage = 'Unknown error';
            if (error instanceof Error) {
                errorMessage = error.message;
            } else if (typeof error === 'string') {
                errorMessage = error;
            }
            alert(`Error: ${errorMessage}`);
        }
    });
    
    /**
     * Allow pressing Enter key in the search input to trigger search
     */
    queryInput.addEventListener('keyup', function(event: KeyboardEvent) {
        if (event.key === 'Enter') {
            searchButton.click();
        }
    });
    
    // Add CSS class to enable transitions after page load
    setTimeout(() => {
        document.body.classList.add('transitions-enabled');
    }, 300);
}); 